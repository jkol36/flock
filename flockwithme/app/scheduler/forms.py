...
class JobCreationForm(forms.Form):
	...
	def clean(self):
		...
		try:
			...
			self.cleaned_data['list_id'] = self.cleaned_data['list_id'] or None
	...
	def is_valid(self):
		...
		try: 
			if self.cleaned_data['list_id']:
				self.list_name= TwitterList.objects.get(pk=list_id)
			else:
				self.list_name = None
	...
	def save(self):
		...
		job.twitter_list = self.cleaned_data.get('list_id')
